                                        ...
                                        <Link to={`/home/products/${item._id}`}>
                                            <img src={item.image} alt=""/>
                                             <div className="flex justify-between mt-10">
                                                ...
                                                <span className="text-3xl text-red-500 font-bold">&#36; {priceFormat(item.price)}</span>
                                            </div>
                                        </Link>
                                      
                                        <div className="flex justify-center mt-10 mx-2">
                                            <button onClick={() => handleButton('buy-now', item)} className="text-white bg-red-500 hover:bg-red-600 mr-3 w-full text-center py-2 px-3 rounded">Buy Now</button>
                                            <button onClick={() => handleButton('add-cart', item)} className="text-white bg-yellow-500 hover:bg-yellow-600 w-full text-center py-2 px-3 rounded">Add to Cart</button>
                                        </div>
                                    </div>
                                ))
                            }
                        </div>
                    : <h1 className="my-5 text-center font-bold text-lg">-- NO PRODUCTS FOUND -- </h1>
           }
        </div>
    )
}
